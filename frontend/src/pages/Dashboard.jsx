...
const Dashboard = () => {
  ...
  useWebSocket((wsData) => {
    ...
    performanceOptimizer.debounce('wsUpdate', () => {
      ...
      if (wsData.paths) updates.paths = { ...state.paths, ...wsData.paths };
      if (wsData.predictions) updates.predictions = { ...state.predictions, ...wsData.predictions };
      if (wsData.threats) updates.threats = { ...state.threats, ...wsData.threats };
      if (wsData.clusters) updates.clusters = { ...state.clusters, ...wsData.clusters };
      if (wsData.fused_intelligence) updates.fused = { ...state.fused, ...wsData.fused_intelligence };
  ...
  const fetchInitialData = async () => {
    try {
      ...
      addChatMessage('error', 'Failed to connect to backend. Make sure server is running on localhost:8000');
    }
  };
  ...
  const handleThreatClick = useCallback((objectId, threat) => {
    addChatMessage('system', `🎯 Selected threat: Object ${objectId} - ${threat.level} level threat with ${(threat.score * 100).toFixed(1)}% confidence`);
    
    // You can add logic to center map on this object
    if (state.paths && state.paths[objectId]) {
      const lastPoint = state.paths[objectId][state.paths[objectId].length - 1];
      console.log(`📍 Object ${objectId} location:`, lastPoint);
    }
  }, [state.paths]);
  ...
  const handleGraphNodeClick = useCallback((node) => {
    if (node.type === 'vehicle') {
      addChatMessage('system', `🔍 Vehicle ${node.objectId} selected - ${node.fused?.threat?.level || 'No threat'} level`);
    } else if (node.type === 'sensor') {
      addChatMessage('system', `📡 Sensor ${node.sensorData.sensor_id} selected - Speed: ${node.sensorData.speed || 'N/A'} m/s`);
    } else if (node.type === 'cluster') {
      addChatMessage('system', `👥 Cluster ${node.clusterId} selected - Contains multiple vehicles`);
    }
  }, []);
      ...
      <div className="dashboard-header">
        ...
        <div className="header-controls">
          <div className="upload-section">
            ...
            >
              {isUploading ? '📤 UPLOADING...' : '🎥 UPLOAD DRONE VIDEO'}
            </button>
            {isUploading && (
          ...
          <div className="chat-section">
            ...
            <div className="chat-messages">
              {chatHistory.length === 0 ? (
                ...
                chatHistory.map(msg => (
                  <div key={msg.id} className={`chat-message ${msg.type} ${msg.isTyping ? 'typing' : ''}`}>
            ...
            <div className="quick-actions">
              ...
              <button onClick={() => setChatInput("Show object count")}>
                Object Count
              </button>
            </div>
          </div>
