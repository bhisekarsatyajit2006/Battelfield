...
// Satellite API
export const getSatelliteFeed = async (band = 'optical') => {
  try {
    const response = await api.get('/satellite/feed', { params: { band } });
    return response.data;
  } catch (error) {
    console.error('Error fetching satellite feed:', error);
    throw error;
  }
};

export const triggerSatelliteScan = async (band = 'optical') => {
  try {
    const response = await api.post('/satellite/scan', { band });
    return response.data;
  } catch (error) {
    console.error('Error triggering satellite scan:', error);
    throw error;
  }
};
